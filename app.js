...
// lines with a write in the air: the poll must not draw the database's older copy over them
...
// only the columns an edit actually touched go up. A whole row would carry this computer's
// 30-second-old status back over one the portal changed a moment ago — the quiet revert that
// merging the old file used to prevent.
const FIELD_COLS = { qty: ['quantity'], status: ['status'], paid: ['paid'], name: ['item_name'],
  client: ['notes'], phone: ['notes'], note: ['notes'],
  supplier: ['item_name', 'supplier'], cat: ['item_name', 'cat'] };
function pick(row, field) {
  const cols = FIELD_COLS[field];
  if (!cols) return row;
  const out = { updated_at: row.updated_at };
...
async function saveOrders(ids, field) {
  if (!clinicMode) {
  ...
  for (const id of ids) {
    ...
    try {
      ...
      else await ordApi('/' + id, 'PUT', pick(toRow(l), field));
    } catch (e) {
      ok = false;
      toast('⚠️ השמירה נכשלה (' + e.message + ') — "' + (l.name || '') + '" לא נשמר', true);
    }
  }
  ids.forEach(id => saving.delete(id));
  // the row on screen still carries the local id it was drawn with, and a click on it would look
  // up a line that no longer answers to that name — but never redraw under someone's fingers
  if (renumbered && !$('#ordList').contains(document.activeElement)) renderOrders();
  return ok;
}
...
function ordPoll() {
  ...
  ordTimer = setTimeout(async () => {
    if (!$('#orders').hidden && document.visibilityState === 'visible') {
      ...
      try {
        const rows = await ordFetch();
        // a write still in the air, and a line not yet given its database id, stay as they are
        const mine = O.lines.filter(l => saving.has(l.id) || /^o-/.test(l.id));
        const held = new Set(mine.map(l => l.id));
        O.lines = rows.filter(r => !held.has(r.id)).concat(mine);
      } catch {}
...
function setLine(id, field, val) {
  ...
  renderOrders(); saveOrders([id], field);
}
...
async function markSheetOrdered() {
  ...
  renderOrders();
  await saveOrders(hit.map(l => l.id), 'status');
...
if (typeof module !== 'undefined' && module.exports)
  module.exports = { fromRow, toRow, pick, sortLines, sheetText, _pricing: { P, params, calc, bandFor, setRow } };
})();
